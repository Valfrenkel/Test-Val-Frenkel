...
function startRecursive() {
  ...
  for (let i = 0; i < usersFetch; i++) {
    usersScope.push(processUser(users[i].id));
  }

  users.splice(0, usersFetch);
  if (users.length !== 0) {
    return startRecursive();
  }

  return Promise.all(usersScope).then( userData => {
    return userData.reduce((previousUser, currentUser) => previousUser + currentUser);
  })
}
